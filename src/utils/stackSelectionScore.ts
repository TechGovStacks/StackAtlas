...
export function computeStackSelectionAssessment(input: StackSelectionAssessmentInput): StackSelectionAssessmentResult {
	...
	const weightedScore = (Object.entries(STACK_SELECTION_WEIGHTS) as [StackSelectionDimensionKey, number][]).reduce(
		(sum, [key, weight]) => sum + normalizedDimensions[key] * weight,
		0,
	);
