...
// Hybrid Scoring Scale: 6 Categories. Integer labels stay documented as
// 0-30, 31-45, ...; decimal stack averages are mapped continuously.
...
function getScoreCategoryData(score: number): (typeof SCORE_CATEGORIES)[number] {
	const clampedScore = clampScore(score);
	return SCORE_CATEGORIES.find((c) => clampedScore <= c.max) ?? SCORE_CATEGORIES[0];
}
...
export function getScoreCategory(score: number): SovereigntyScoreCategory {
	return getScoreCategoryData(score).category;
}
...
export function getScorePercentileInCategory(score: number): number {
	const clampedScore = clampScore(score);
	const categoryIndex = SCORE_CATEGORIES.findIndex((c) => clampedScore <= c.max);
	const categoryData = SCORE_CATEGORIES[categoryIndex] ?? SCORE_CATEGORIES[0];
	const lowerBound = categoryIndex > 0 ? SCORE_CATEGORIES[categoryIndex - 1].max : categoryData.min;

	const range = categoryData.max - lowerBound;
	if (range === 0) return 50; // Shouldn't happen but safety check

	const positionInRange = clampedScore - lowerBound;
...
export function getScoreCategoryColor(score: number): string {
	return getScoreCategoryData(score).color;
}

export function getScoreCategoryColorByCategory(category: SovereigntyScoreCategory): string {
	return SCORE_CATEGORIES.find((c) => c.category === category)?.color ?? SCORE_CATEGORIES[0].color;
}
