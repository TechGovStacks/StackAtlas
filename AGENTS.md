...
```bash
# Install dependencies (also runs postinstall: asset copy + data generation)
pnpm i
...
- `scripts/` – Build-time generators (catalog/data assets)
...
- Do not manually edit `src/data/items.generated.ts` – it is generated by `scripts/generate-data.mjs` (via `pnpm prebuild`/`postinstall`) using `src/data/logo-urls.json` for fallback resolution.
- All item logos are guaranteed to display: external URL, local asset, or `/assets/broken-logo.svg` (for items without public logo).
