...
import { expect, test, type Page } from './playwrightSafe';
...
test('starts in French for fr-FR', async ({ page }) => {
	await gotoApp(page, '/?lng=fr');
	await expectCoreTranslations(page, 'fr');
	await expect(page).toHaveScreenshot('i18n-fr-initial.png', { fullPage: true });
});

test('falls back to German for unsupported language query', async ({ page }) => {
	await gotoApp(page, '/?lng=pt-BR');
