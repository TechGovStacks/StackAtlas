...
| Sehr Gut     | 76-90  | 🟢 Grün       | Sehr empfohlen, sehr hohe Souveränität |
| Hervorragend | 91-100 | 🟢 Dunkelgrün | Gold-Standard, optimale Souveränität   |

Hinweis für berechnete Durchschnittswerte: Stack-Scores können Dezimalstellen
haben. Die Implementierung wertet die Skala kontinuierlich anhand der oberen
Grenzen aus, damit Werte wie 90,1 nicht in eine Lücke zwischen 90 und 91 fallen.
...
baseScore =
	(openSource ? 30 : 0) + (euHeadquartered ? 25 : 0) + (hasAudit ? 20 : 0) + (permissiveLicense ? 10 : 0) + (matureProject ? 10 : 0) + (largeEcosystem ? 5 : 0);
...
if (numericScore <= 30)
	...
	category = 'good'; // 🟢 (hellgrün)
else if (numericScore <= 90)
	category = 'excellent'; // 🟢 (grün)
